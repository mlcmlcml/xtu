...
// 修改1: 使用正确的API导入
import forumApi from "@/api/forum.js";  // 确保路径正确
...
export default {
  ...
  methods: {
    ...
    getAclhotList() {
      forumApi.getHotArticles()
        .then((response) => {
          // 修正1: 使用 response.data 获取实际数据
          if (response.data.code === 20000) {
            this.hotList = response.data.data;
          } else {
            this.$message.error("获取热帖失败: " + response.data.message);
          }
        })
        .catch(error => {
          console.error("获取热帖失败:", error);
          this.$message.error("获取热帖数据失败");
        });
    },
    getAclTaghotList() {
      forumApi.getHotTags()
        .then((response) => {
          // 修正2: 使用 response.data 获取实际数据
          if (response.data.code === 20000) {
            this.hotTagList = response.data.data;
          } else {
            this.$message.error("获取热门话题失败: " + response.data.message);
          }
        })
        .catch(error => {
